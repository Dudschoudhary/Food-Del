...
const ExploreMenu = ({ category, setCategory }) => {
  return (
    <div className='flex flex-col gap-4 lg:text-sm' id='exploreMenu'>
      ...
      <div className="explore-menu flex justify-between items-center gap-[40px] sm:gap-[30px] text-center mx-[20px] overflow-x-scroll">
        {menu_list.map((item, index) => {
          return (
            <div onClick={()=> setCategory(prev => prev === item.menu_name ? "All" : item.menu_name)} className='explore-list-menu-item' key={index}>
              <img src={item.menu_image} className={ category === item.menu_name ? "active" : "gap-8 bg-red-600" } alt="" />
